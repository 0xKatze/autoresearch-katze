#!/usr/bin/env python3
"""
attack_diffusion_v2.py — N23-v2 (Bounded-magnitude DDIM)

v2 hypothesis: N23-NoParam (v1) was killed because the cosine DDIM schedule
sends predict_x0_from_xt through huge-magnitude states (division by sqrt(alpha_bar)
near 0). After construct_perturbed_graph multiplies by feat_scale, the GCN
saturates and grad_{x0} CW becomes uninformative. v2 restricts the schedule
to alpha in [0.5, 0.99] so x_t and x0_pred stay in the non-saturating regime
where score guidance is informative.
...
# Reuse helpers from baseline. attack_baseline.py is a frozen snapshot of the
# original generator-based KATZE attack.py — kept here so the file swap that
# puts this module in attack.py's place doesn't create a self-import loop.
...
CONFIG = {
    # Diffusion schedule — v2: BOUNDED to keep x_t / x0_pred non-saturating
    "T_steps": 50,                # reverse-process steps (replaces attack_epochs)
    "schedule": "linear",         # linear pairs well with narrow alpha band
    "alpha_min": 0.5,             # alpha at t=T — keeps sqrt((1-α)/α) <= 1
    "alpha_max": 0.99,            # alpha at t=0
...
if __name__ == "__main__":
    print("attack_diffusion_v2.py — N23-v2 (bounded-magnitude DDIM)")
    print(f"CONFIG: schedule={CONFIG['schedule']}, "
          f"alpha_min={CONFIG['alpha_min']}, alpha_max={CONFIG['alpha_max']}, "
          f"T_steps={CONFIG['T_steps']}, guidance={CONFIG['guidance_scale']}, "
          f"feat_scale={CONFIG['feat_scale']}")
    alphas = make_alpha_schedule(
        CONFIG["T_steps"], mode=CONFIG["schedule"],
        alpha_min=CONFIG["alpha_min"], alpha_max=CONFIG["alpha_max"])
    print(f"alpha schedule: alpha[0]={alphas[0]:.4f} (clean), "
          f"alpha[T]={alphas[-1]:.4f} (noisy), alpha[T/2]={alphas[len(alphas)//2]:.4f}")
    import torch
    ratio = (torch.sqrt(1 - alphas) / torch.sqrt(alphas)).max().item()
    print(f"max sqrt((1-α)/α) = {ratio:.3f}  "
          f"(this is the x0_pred magnification factor; v1 was huge, v2 should be <= 1)")
